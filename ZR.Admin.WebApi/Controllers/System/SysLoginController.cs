...
namespace ZR.Admin.WebApi.Controllers.System
{
    ...
    public class SysLoginController : BaseController
    {
        ...
        public async Task<IActionResult> Login([FromBody] LoginBodyDto loginBody)
        {
            ...
            return SUCCESS(await SaveOrGetUserInCache(user, loginUser, permissions));
        }

        async Task<string> SaveOrGetUserInCache(SysUser user, TokenModel loginUser, List<string> permissions)
        {
            var uid = user.UserId.ToString();
            var jwtToken = JwtUtil.GenerateJwtToken(JwtUtil.AddClaims(loginUser));

            if (!await CacheService.CheckUserToken(uid))
            {
                using var _ = await CacheService.UseSessionCacheLock();

                if(!await CacheService.CheckUserToken(uid))
                {
                    await CacheService.SetUserToken(uid, loginUser);
                    await CacheService.SetUserPerms(GlobalConstant.UserPermKEY + user.UserId, permissions);
                }
            }
        ...
        public async Task<IActionResult> LogOut()
        {
            ...
            await CacheService.RemoveUserPerms(GlobalConstant.UserPermKEY + userid);
            await CacheService.RemoveUserToken(userid.ToString());
        ...
        [Log(Title = "扫码登录")]
        [Verify]
        public async Task<IActionResult> ScanLogin([FromBody] ScanDto dto)
        {
            ...
            TokenModel tokenModel = await JwtUtil.GetLoginUser(HttpContext);
