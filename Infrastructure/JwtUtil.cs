...
namespace Infrastructure
{
    ...
    public class JwtUtil
    {
        ...
        /// <returns></returns>
        public static async Task<TokenModel> GetLoginUser(HttpContext httpContext)
        {
            string token = httpContext.GetToken();

            if (!string.IsNullOrEmpty(token))
            {
                return await ValidateJwtToken(ParseToken(token));
            }
            return null;
        }
        ...
        /// <returns></returns>
        public static async Task<TokenModel?> ValidateJwtToken(JwtSecurityToken jwtSecurityToken)
        {
            try
            {
                ...
                var uid = claims.FirstOrDefault(x => x.Type == "primarysid")?.Value;

                var loginUser = await CacheService.GetUserToken(uid);

                if (loginUser != null)
                {
                    loginUser.JwtExpire = jwtSecurityToken.ValidTo;
                }
                return loginUser;
            }
            catch (Exception ex)
