            ...
            foreach (var type in assembly.GetTypes())
            {
                ...
                if (serviceAttribute != null)
                {
                    ...
                    if (serviceType == null && serviceAttribute.InterfaceServiceType)
                    {
                        ...
                        var rFaces = type.GetInterfaces().Except(baseInterfaces);

                        var facesToRemove = new HashSet<Type>();
                        foreach(var face in rFaces)
                        {
                            foreach (var f in face.GetInterfaces())
                                facesToRemove.Add(f);
                        }

                        rFaces = rFaces.Except(facesToRemove);

                        serviceType = rFaces.FirstOrDefault();
                    }
