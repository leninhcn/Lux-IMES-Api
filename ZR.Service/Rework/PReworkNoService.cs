...
AND A.WORK_ORDER = C.WORK_ORDER ) ORDER BY MES_SPEC";
        ...
        {
            var sql=$@"     
     WITH SRC AS(SELECT ROUTE_NAME,STATION_TYPE,NEXT_STATION_TYPE FROM SAJET.M_ROUTE_DETAIL WHERE ROUTE_NAME='{parm}' AND NECESSARY='Y' AND RESULT=0)
