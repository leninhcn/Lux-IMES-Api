...
namespace ZR.Service.Repair
{
    ...
    public class RepairPanelService : BaseService<SnStatus>, IRepairPanelService
    {
        public async Task<ExecuteResult> getSNByPanel(RepairPanel panel)
        {
            ...
            try
            {
                string sSQL = @"select  /*+RULE*/SERIAL_NUMBER, OUT_STATIONTYPE_TIME  from IMES.P_SN_STATUS a
where A.PANEL_NO = @PANEL_NO  AND A.WORK_FLAG = '0' AND A.CURRENT_STATUS= '1'
AND EXISTS (SELECT * FROM imes.m_ROUTE_DETAIL WHERE NEXT_STATION_TYPE = @NEXT_STATION_TYPE and STATION_TYPE = a.STATION_TYPE and ROUTE_NAME = a.ROUTE_NAME)";
        ...
        public async Task<ExecuteResult> getPanelByStationType(string stationType)
        {
            ...
            try
            {
                string sSQL = string.Format(@"select  /*+RULE*/DISTINCT(A.PANEL_NO) from IMES.P_SN_STATUS a
                    where  A.PANEL_NO <> 'N/A'
                    AND A.WORK_FLAG = '0'
                    AND A.CURRENT_STATUS= '1'
                    AND EXISTS 
                        (SELECT * FROM imes.m_ROUTE_DETAIL WHERE NEXT_STATION_TYPE = '{0}' and STATION_TYPE = a.STATION_TYPE and ROUTE_NAME = a.ROUTE_NAME)", stationType);
        ...
        public async Task<ExecuteResult> geInfoBySN(string sn)
        {
            ...
            try
            {
                string sSQL = @"select A.SERIAL_NUMBER,A.WORK_ORDER, b.ipn, a.OUT_STATIONTYPE_TIME
                    from IMES.P_SN_STATUS a, imes.m_part b
        ...
        {
            string sSQL = string.Format(@"select a.NEXT_STATION_TYPE  from IMES.P_SN_STATUS a where a.PANEL_NO ='{0}'  AND a.NEXT_STATION_TYPE <> '0' and rownum = 1", panel);
        ...
        public  async Task<bool> repairPanel(panelDto currStation,string sNextStationType)
        {
            ...
            string sSQL = string.Format(@"update IMES.P_SN_STATUS a set a.LINE =@line, a.STAGE = @stage, a.STATION_TYPE = @station_type, a.STATION_NAME = @station_name, 
