    ...
    public class PHoldOptionsService : BaseService<PHoldOptionsDto>, IHoldOptionsService
    {
        public Dictionary<string,List<PHoldOptionsDto>> GetPHoldOptionsDto()
        {
            var sql = " WITH METHODOPTIONS "
                    ...
                    +",STATIONOPTIONS "
                    +" AS(SELECT DISTINCT STATION_TYPE NAME,STATION_TYPE VALUE,0 STATUS FROM SAJET.M_STATION_TYPE) "
                    +",STAGEOPTIONS "
                    +" AS(SELECT DISTINCT STAGE NAME,STAGE VALUE,0 STATUS FROM SAJET.M_STATION_TYPE) "
