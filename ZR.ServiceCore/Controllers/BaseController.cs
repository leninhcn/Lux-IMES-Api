...
namespace ZR.ServiceCore.Bascon
{
    /// <summary>
    /// web层通用数据处理
    /// </summary>
    public class BaseController : ControllerBase
    {
        ...
        public readonly IMResponErrorCodeService _ResponErrorCodeService;

        //构造函数
        public BaseController( ) 
        {
            _ResponErrorCodeService = App.GetService<IMResponErrorCodeService>();
        }
        ...
        /// <param name="apiResult"></param>
        /// <returns></returns>
        protected async Task< IActionResult> ToResponseByErrorcode(ResponseErrorCodeDto parm,ApiResult apiResult)
        {
            if (apiResult["code"].ParseToInt()!=200 && apiResult["msg"].ToString().StartsWith("SAJETERR"))
            {
                var item = apiResult["msg"].ToString().Split('_');
                parm.ResponseErrorCode = item[0];
                parm.Param = item.Length > 1 ? item[1] : "";
                apiResult["msg"] = await _ResponErrorCodeService.GetResponseMsg(parm);
            }
        ...
        /// <returns></returns>
        protected async Task<IActionResult> ToResponseByErrorcode<T>(Resultinfo<T> resultinfo)
        {
            
            if (resultinfo.Result == "OK")
            {
                resultinfo.ResMSG = "Sucess" + (resultinfo.ResMSG.IsNullOrEmpty() ? "" : (","+resultinfo.ResMSG));
            }
            else
            {
                if( resultinfo.GetResMSG.StartsWith("SAJETERR"))
                {
                    //var item = resultinfo.GetResMSG.ToString().Split('_');
                    ResponseErrorCodeDto parm = new ResponseErrorCodeDto()
                    {
                        Lang = resultinfo.Lange,
                        ResponseErrorCode = resultinfo.Result,
                        Param = resultinfo.ResErrCodeParam
                    };
                    resultinfo.ResMSG = await _ResponErrorCodeService.GetResponseMsg(parm)+resultinfo.ResMSG;
                }
               else
                {
                    resultinfo.ResMSG = resultinfo.Result +  resultinfo.ResMSG;
                }
            }

            string jsonStr = GetJsonStr(new ApiResult(resultinfo.Result == "OK" ? (int)ResultCode.SUCCESS : (int)ResultCode.CUSTOM_ERROR,resultinfo.ResMSG, resultinfo.Data), TIME_FORMAT_FULL);
        ...
        /// <param name="fileName">带扩展文件名</param>
        /// <returns></returns>
        protected IActionResult Bytes2File(byte[] bytes, string fileName)
        {
        
            Response.Headers.Add("Access-Control-Expose-Headers", "Content-Disposition");
            return File(bytes,  "application/octet-stream", HttpUtility.UrlEncode(fileName));
        }
        #region 方法
