...
namespace ZR.ServiceCore.Middleware
{
    ...
    public class ActionPermissionFilter : ActionFilterAttribute//, IAsyncActionFilter
    {
        ...
        {
            TokenModel info = await JwtUtil.GetLoginUser(context.HttpContext);

            if (info != null && info?.UserId > 0)
            {
                long userId = info.UserId;
                List<string> perms = await CacheService.GetUserPerms(GlobalConstant.UserPermKEY + userId);
                ...
                if (perms == null)
                {
                    ...
                    await CacheService.SetUserPerms(GlobalConstant.UserPermKEY + userId, perms);
                }
