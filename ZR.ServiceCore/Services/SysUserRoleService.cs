...
namespace ZR.Service.System
{
    ...
    public class SysUserRoleService : BaseService<SysUserRole>, ISysUserRoleService
    {
        ...
        {
            return Context.Queryable<SysUserRole, SysUser, ZrSysUser>((t1, u, u2) => new JoinQueryInfos(
                   JoinType.Left, t1.UserId == u.UserId,
                   JoinType.Left, u.UserId == u2.MainId))
                .WithCache(60 * 10)
                .Where((t1, u, u2) => t1.RoleId == roleId && u2.DelFlag == 0)
                .Select((t1, u) => u)
                .ToList();
        }
        ...
        {
            var query = Context.Queryable<SysUserRole, SysUser, ZrSysUser>((t1, u, u2) => new JoinQueryInfos(
                JoinType.Left, t1.UserId == u.UserId,
                JoinType.Left, u.UserId == u2.MainId))
                .Where((t1, u, u2) => t1.RoleId == roleUserQueryDto.RoleId && u2.DelFlag == 0);
