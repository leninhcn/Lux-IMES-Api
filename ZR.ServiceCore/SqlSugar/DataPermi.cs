...
namespace ZR.ServiceCore.SqlSugar
{
    ...
    public class DataPermi
    {
        ...
        {
            //获取当前用户的信息
            var user = JwtUtil.GetLoginUser(App.HttpContext).Result;
            if (user == null) return;
            //管理员不过滤
            ...
            foreach (var role in user.Roles.OrderBy(f => f.DataScope))
            {
                ...
                else if (DataPermiEnum.CUSTOM.Equals(dataScope))//自定数据权限
                {
                    ...
                    expUser.Or(it => SqlFunc.Subqueryable<ZrSysUser>()
                    .InnerJoin<SysRoleDept>((f, d)=> f.DeptId == d.DeptId)
                    .Where((f, d) => it.UserId == f.MainId && d.RoleId == role.RoleId).Any());
                }
                else if (DataPermiEnum.DEPT.Equals(dataScope))//本部门数据
                {
                    expUser.Or(it => SqlFunc.Subqueryable<ZrSysUser>().Where(d=> d.MainId == it.UserId && d.DeptId == user.DeptId).Any());
                }
                else if (DataPermiEnum.DEPT_CHILD.Equals(dataScope))//本部门及以下数据
                {
                    ...
                    expUser.Or(it => SqlFunc.Subqueryable<ZrSysUser>()
                    .Where(u2=> u2.MainId == it.UserId && allChildDepts.Select(f => f.DeptId).ToList().Contains(u2.DeptId)).Any());
                }
